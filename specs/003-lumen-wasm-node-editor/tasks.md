...
- [x] T056 [US7] Create toolbar component in `apps/editor/src/components/toolbar.tsx` — Export JSON button (triggers serialize → download), Import JSON button (triggers file picker → deserialize → load into store), render settings controls (width, height, background color, fps, duration). Validate on import, show errors inline.
