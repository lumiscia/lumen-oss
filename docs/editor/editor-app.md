...
- Build + sync the wasm bundle before previewing: `pnpm wasm:build` (requires wasm-pack + wasm-bindgen CLI).
  - Copies artifacts (`lumen_wasm.js`, `lumen_wasm_bg.js`, `lumen_wasm_bg.wasm`, `lumen_wasm.d.ts`) into `packages/lumen/src/generated/lumen-wasm/` so consumers import wasm from the package bundle path.
