...
use lumen::{
	...
	graph::Graph,
	node::{
		crop::Crop, media_output::MediaOutput, shape::Shape, solid_color::SolidColor, switch::Switch,
		transform::Transform, Node, ShapeGeometry,
	},
...
fn cycle_detection_rejects_cyclic_graph() {
	...
	let a = graph.add_node(Node::new(NodeId(0), NodeKind::Transform(Transform::default())));
...
fn missing_required_input_rejects_graph() {
	...
	let errors = graph.validate().expect_err("transform source input is required");
	assert!(errors.iter().any(|error| {
		matches!(
			error,
...
fn port_type_mismatch_rejects_graph() {
	...
	let errors = graph.validate().expect_err("vector to raster connection should fail");
	assert!(errors.iter().any(|error| {
		matches!(
			error,
...
fn valid_solid_transform_media_output_graph_passes_validation() {
	let mut graph = Graph::new();
	let solid = graph.add_node(Node::new(NodeId(0), NodeKind::SolidColor(SolidColor::default())));
	let transform = graph.add_node(Node::new(NodeId(0), NodeKind::Transform(Transform::default())));
	let output = graph.add_node(Node::new(NodeId(0), NodeKind::MediaOutput(MediaOutput)));
...
fn overlapping_switch_ranges_are_rejected() {
	...
	let errors = graph.validate().expect_err("switch overlap must fail validation");
	assert!(errors.iter().any(|error| {
		matches!(
			error,
