...
use lumen::{
	...
	node::{media_output::MediaOutput, solid_color::SolidColor, transform::Transform, Node, PropertyValue},
	AnimatableType, Composition, Connection, Extrapolation, Graph, InputPort, InterpolationMode,
	AssetCache, Keyframe, KeyframeTrack, NodeId, NodeKind, NullMediaStore, OutputPort,
...
fn base_composition() -> (Composition, NodeId) {
	...
	let transform = graph.add_node(Node::new(NodeId(0), NodeKind::Transform(Transform::default())));
...
fn hold_extrapolation_before_and_after_key_range() {
	...
	assert_eq!(track.sample(0).expect("before range"), PropertyValue::Float(10.0));
	assert_eq!(track.sample(30).expect("after range"), PropertyValue::Float(20.0));
}
...
fn single_key_track_returns_same_value_for_all_frames() {
	...
	assert_eq!(track.sample(0).expect("sample 0"), PropertyValue::Float(0.25));
	assert_eq!(track.sample(15).expect("sample 15"), PropertyValue::Float(0.25));
	assert_eq!(track.sample(90).expect("sample 90"), PropertyValue::Float(0.25));
}
...
fn keyframed_transform_affects_rendered_frame_positions() {
	...
	let start = composition.render_frame(0, &mut context).expect("frame 0 render");
	let end = composition.render_frame(30, &mut context).expect("frame 30 render");
