...
impl NodeEval for Transform {
	...
	fn evaluate(&self, inputs: &NodeInputs, _ctx: &mut RenderContext) -> Result<PortValue, LumenError> {
		...
		let tx = self.translate_x.round() as i32;
		let ty = self.translate_y.round() as i32;
...
impl Transform {
	fn has_non_translation_transform(&self) -> bool {
		let scale_is_identity = (self.scale_x - 1.0).abs() <= f32::EPSILON
			&& (self.scale_y - 1.0).abs() <= f32::EPSILON;
		let rotation_is_identity = self.rotate.abs() <= f32::EPSILON;
		let pivot_is_origin = self.pivot_x.abs() <= f32::EPSILON && self.pivot_y.abs() <= f32::EPSILON;
