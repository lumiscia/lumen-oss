...
impl Composition {
	pub fn render_frame(&self, frame: u32, ctx: &mut RenderContext) -> Result<RasterFrame, LumenError> {
		...
		for node_id in order {
			...
			let node = self.graph.nodes.get(&node_id).ok_or(
				GraphValidationError::InvalidEvaluationTarget { node_id },
			)?;
			...
			for input_def in node.kind.input_port_defs() {
				let upstream_edge = self.graph.connections.iter().find(|edge| {
					edge.to_node == node_id
						&& input_port_matches(&edge.to_port, input_def.name)
				});
			...
			let output = node.kind.evaluate(&inputs, ctx).map_err(|err| {
