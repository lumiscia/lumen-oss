...
fn resolve_scalar(
	...
	match scalar {
		...
		Scalar::Expr(s) => {
			...
			eval_expr(&parsed, index).map(|v| v * scale).map_err(|e| CompileError::ExprError {
				layer_id: layer_id.to_string(),
				clip_id: clip_id.to_string(),
...
fn try_resolve_scalar_for_index(
	...
	match scalar {
		Scalar::Literal(v) => Ok(Some(v * scale)),
		Scalar::Expr(s) => {
			let parsed = parse_expr(s).map_err(|e| CompileError::ExprError {
				layer_id: layer_id.to_string(),
				clip_id: clip_id.to_string(),
...
fn build_clip_property_index(
	project: &Project,
	scale: f32,
) -> Result<ClipPropertyIndex, CompileError> {
	...
	let mut index = ClipPropertyIndex {
		canvas_width: project.canvas.width as f32 * scale,
		canvas_height: project.canvas.height as f32 * scale,
		clips: HashMap::new(),
	};
	...
	loop {
		...
		for key in keys {
			...
			let rx = try_resolve_scalar_for_index(
				&layer_id, &key, &entry.x, scale, &index, &all_ids,
			)?;
			let Some(rx) = rx else { continue };

			// Re-borrow after potential index mutation
			let entry = unresolved.get(&key).unwrap();
			let ry = try_resolve_scalar_for_index(
				&layer_id, &key, &entry.y, scale, &index, &all_ids,
			)?;
			...
			let rw = match &entry.width {
				None => None,
				Some(w) => {
					let resolved = try_resolve_scalar_for_index(
						&layer_id, &key, w, scale, &index, &all_ids,
					)?;
			...
			let rh = match &entry.height {
				...
				Some(h) => {
					let resolved = try_resolve_scalar_for_index(
						&layer_id, &key, h, scale, &index, &all_ids,
					)?;
					let Some(v) = resolved else { continue };
					Some(v)
				}
...
pub fn compile_project_with_scale(
	...
	let clip_index = build_clip_property_index(project, scale)?;
...
fn compile_layer_item(
	...
	match item {
		...
		LayerItem::Group(group) => {
			...
			let transform =
				compile_group_transform(&layer.id, &group.id, &group.transform, ctx.scale, ctx.clip_index)?;
...
fn compile_clip_operation(
	...
	let kind = compile_clip_content(layer.id.as_str(), clip, ctx.sources, ctx.scale, ctx.clip_index)?;
	...
	if !transform.x.is_finite() || !transform.y.is_finite() || !transform.rotation_degrees.is_finite()
	{
		return Err(CompileError::InvalidClip {
			layer_id: layer.id.clone(),
			clip_id: clip.id.clone(),
		...
		Scalar::Expr(s) => {
			...
			for r in &refs {
				...
				if node_ids.contains(target.as_str()) {
					// Contains at least one layout node ref → keep as deferred expr.
			...
			// If any ref is a node id, return the expr as-is (renderer handles it).
			let has_node_ref = refs.iter().any(|r| node_ids.contains(r.target.as_str()));
			if has_node_ref {
				return Ok(Some(Scalar::Expr(s)));
			}
...
fn compile_layout_edges(
	...
	Ok(LayoutEdges {
		...
		right: compile_layout_edge(
			layer_id, clip_id, node_path, field, "right", edges.right, scale,
		)?,
		bottom: compile_layout_edge(
			layer_id, clip_id, node_path, field, "bottom", edges.bottom, scale,
		)?,
		left: compile_layout_edge(layer_id, clip_id, node_path, field, "left", edges.left, scale)?,
	})
}
...
mod tests {
	use crate::{
		compile::{CompileError, CompiledOperationKind, compile_project, compile_project_with_scale},
		expr::Scalar,
		model::{
			Canvas, Clip, ClipAnimation, ClipContent, ClipGroup, ColorRgba, Easing, Layer,
			LayerItem, LayoutClip, LayoutNode, LayoutNodeKind, LayoutNodeStyle, LayoutTextNode, Project,
	...
	fn canvas_width_expr_resolves_in_transform() {
		let project = Project {
			canvas: Canvas { width: 640, height: 360, background: ColorRgba(0, 0, 0, 255) },
			timeline: Timeline { fps: Rational::new(30, 1).expect("fps"), total_frames: 30 },
			sources: vec![],
			layers: vec![Layer {
				id: "layer_a".to_string(),
				z_index: 0,
