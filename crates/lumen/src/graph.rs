...
impl Graph {
	...
	pub fn validate(&self) -> Result<Vec<Warning>, Vec<LumenError>> {
		...
		if media_output_count == 0 {
			...
			errors.push(GraphValidationError::MultipleMediaOutputs {
	...
	pub fn evaluation_order(&self, target: NodeId) -> Result<Vec<NodeId>, LumenError> {
		...
		while let Some(node_id) = stack.pop() {
			...
			for connection in self.connections.iter().filter(|edge| edge.to_node == node_id) {
		...
		for edge in self.connections.iter().filter(|edge| {
			reachable.contains(&edge.from_node) && reachable.contains(&edge.to_node)
		}) {
		...
		while let Some(node_id) = queue.pop_front() {
			...
			for edge in self.connections.iter().filter(|edge| edge.from_node == node_id) {
