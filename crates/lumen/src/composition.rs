...
use crate::{
	...
	graph::Graph,
	node::{
		merge::Merge, solid_color::SolidColor, transform::Transform, NodeId, NodeKind, PropertyValue,
	},
};
...
impl Composition {
	...
	pub fn sample_property(
		...
		static_property_value(&node.kind, property_path).ok_or(PropertyError::MissingProperty {
			node_id,
			property_path: property_path.to_string(),
		}
		.into())
	...
	pub fn apply_animated_properties(
		...
		for track in self.tracks.iter().filter(|track| track.node_id == node_id) {
			let sampled = self.sample_property(node_id, &track.property_path.0, frame)?;
			apply_property(node_kind, &track.property_path.0, sampled);
		}
		Ok(())
	}
}
