...
impl WasmMediaStore {
    ...
    pub fn set_image(&self, source: String, frame: ImageFrame) -> Result<(), &'static str> {
        ...
        self.inner
            ...
            .write()
            .map_err(|_| "media store lock poisoned")?
            .insert(source, StoredImage { metadata, frame: Arc::new(frame) });
        Ok(())
    }
...
impl MediaStore for WasmMediaStore {
    fn get_image_resolver(&self, source: &str) -> Option<Box<dyn ImageResolver>> {
        ...
        Some(Box::new(WasmImageResolver { id: source.to_string(), entry }))
    }
    ...
    fn get_video_resolver(&self, source: &str) -> Option<Box<dyn VideoFrameResolver>> {
        ...
        Some(Box::new(WasmVideoResolver { id: source.to_string(), entry }))
    }
}
...
impl VideoFrameResolver for WasmVideoResolver {
    ...
    fn resolve_frame_image(&self, frame: u32) -> Result<Arc<ImageFrame>, MediaError> {
        self.entry.frames.get(frame).ok_or(MediaError::FrameOutOfRange {
...
impl LumenMediaStore {
    ...
    pub fn new() -> LumenMediaStore {
        LumenMediaStore { store: WasmMediaStore::default() }
    }
    ...
    pub fn clear_videos(&self) -> Result<(), JsValue> {
        self.store.clear_video_frames().map_err(|e| JsValue::from_str(e))
    }

    #[wasm_bindgen(js_name = "clearVideoSource")]
    pub fn clear_video_source(&self, stream_id: &str) -> Result<(), JsValue> {
        self.store.clear_video_frames_for_source(stream_id).map_err(|e| JsValue::from_str(e))
    }
    ...
    pub fn set_image(
        ...
        self.store.set_image(image_id.to_string(), frame).map_err(|e| JsValue::from_str(e))
    }
    ...
    pub fn set_video_metadata(
        ...
        self.store
            .set_video_metadata(
                stream_id.to_string(),
                VideoMetadata { width, height, frame_count },
            )
            .map_err(|e| JsValue::from_str(e))
    }
}
...
mod tests {
    ...
    fn clear_videos_preserves_metadata() {
        ...
        store
            .set_video_metadata(
                "intro".to_string(),
                VideoMetadata { width: 1920, height: 1080, frame_count: 240 },
            )
            .expect("set metadata");
        let frame =
            crate::utils::image_frame_from_rgba(1, 1, vec![255, 0, 0, 255]).expect("frame");
        store.set_video_frame("intro".to_string(), 0, frame).expect("set frame");

        store.clear_video_frames().expect("clear video frames");

        let resolver =
            store.get_video_resolver("intro").expect("video resolver should still exist");
