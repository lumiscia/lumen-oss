...
fn collect_frame_requirements(project: &Project, frame: u32) -> Result<FrameRequirementsPayload, String> {
...
pub extern "C" fn lumen_renderer_frame_requirements_len(renderer_ptr: *mut WasmRenderer) -> usize {
	let renderer = unsafe { renderer_ptr.as_ref() };
	renderer.map(|value| value.last_requirements.len()).unwrap_or(0)
}
