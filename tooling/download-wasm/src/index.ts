...
async function readVersionFile(
  path: string,
  overrides: { baseUrl: string; version: string },
  ...
  if (!baseUrl) {
    throw new Error(`Missing baseUrl in ${path}, --base-url, or LUMEN_WASM_BASE_URL`);
  }
  if (!version) {
...
Downloads the Lumen WASM binary from public R2 storage.

Options:
  --base-url <url>       Public R2 base URL, or LUMEN_WASM_BASE_URL
